...
function transformDriver(driver) {
    ...
    const options = Object.assign({ sessionId: driver.sessionId, protocol: url.protocol ? url.protocol.replace(/:$/, '') : undefined, hostname: url.hostname, port: Number(url.port) || undefined, path: url.pathname, capabilities: driver.capabilities, logLevel: 'silent' }, environment);
    if (!options.port) {
